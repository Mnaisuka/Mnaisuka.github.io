...
TLDmodlist.prototype = {
	...
	loadInitData: function(){
		...
		$.getJSON("https://raw.githubusercontent.com/TLD-Mods/ModLists/master/SiteData.json", function(data) {
			...
			if (data.melonNightly && data.melonNightly == true){
				_this.mlNightly = "<br />(Make sure to tick \"Enable Nightly Builds\" and pick the latest)";
			}
			
			if (data.announcement){
				_this.announcementHtml = data.announcement;
			} else {
				$("#announcement").hide();
			}
			...
			$(".melonloader-nightly").each(function() {
				$(this).html(_this.mlNightly);
			});
			$("#announcement").each(function() {
				$(this).html(_this.announcementHtml);
			});
		});
	},
...
//		this.filters.processURLFilters();
	...
	applySort: function(type, direction){
		...
		if (type == "update"){
			var result = $('#modList .mod-item').sort(function (a, b) {
